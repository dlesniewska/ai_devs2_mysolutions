...
if __name__ == '__main__':
    ...
    if not collectionInfo.points_count:
        # loader = JSONLoader(MEMORY_PATH, '.content')
        # memory = loader.load()[0]
        with open(MEMORY_PATH, 'r', encoding="utf8") as handle:
            data = handle.read()
            memory = [json.loads(l) for l in data.splitlines('}')]

        documents = [Document(page_content=content,metadata={'source':COLLECTION_NAME, 'uuid':str(uuid4()), 'content': content}) for content in memory]
